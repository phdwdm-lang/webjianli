...
void main() {
  ...
  // 涟漪折射强度驱动 alpha：非涟漪区透明(透出下层水彩)，仅涟漪处显示扭曲后的纹理，避免不透明黑底闪烁
  float ripple = smoothstep(0.004, 0.05, amount);
...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    const loop = now => {
      ...
      // 动态纹理源（如 KoiPond 鱼池 canvas）：每帧上传，使鱼/荷叶随涟漪折射
      const sourceCanvas = sourceRef?.current;
      if (sourceCanvas) {
        imageTexture.image = sourceCanvas;
        imageTexture.needsUpdate = true;
        compositeUniforms.uTextureSize.value = [sourceCanvas.width || 1, sourceCanvas.height || 1];
      }
